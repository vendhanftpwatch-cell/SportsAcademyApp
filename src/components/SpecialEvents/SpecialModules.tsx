...
export function SummerCamp() {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-4 md:pb-12">
      {/* Hero Banner */}
      <div className="vibrant-orange rounded-[2rem] sm:rounded-[32px] md:rounded-[40px] p-6 md:p-12 text-white relative overflow-hidden shadow-2xl min-h-[300px] sm:min-h-[350px] md:min-h-[400px] flex flex-col justify-center">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="flex flex-col xs:flex-row gap-3 md:gap-4">
            <button className="bg-white text-secondary px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-sm md:text-base shadow-xl hover:scale-105 transition-transform active:scale-95 min-h-[44px]">REGISTER NOW</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 lg:gap-8">
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
          ...
          <div className="space-y-6">
            {[
              { week: 'Week 1', focus: 'Fundamental Skills & Drills', date: 'July 1st - 7th', color: 'bg-blue-50 text-primary border border-blue-100' },
              { week: 'Week 2', focus: 'Advanced Tactical Training', date: 'July 8th - 14th', color: 'bg-orange-50 text-rose-500 border border-orange-100' },
              { week: 'Week 3', focus: 'Internal Academy League', date: 'July 15th - 21th', color: 'bg-purple-50 text-accent border border-purple-100' },
            ].map((item, i) => (
              <div key={i} className="flex gap-6 p-4 rounded-3xl hover:bg-indigo-50/50 transition-colors group border border-transparent hover:border-indigo-100">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-indigo-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div className="mt-8 p-6 bg-slate-50 rounded-3xl shadow-sm border border-slate-100">
             <h4 className="font-bold text-slate-800 mb-2">Age Categories</h4>
             <div className="flex flex-wrap gap-2">
               {['Under-10', 'Under-13', 'Under-16', 'Pro-College'].map(cat => (
                 <span key={cat} className="px-3 py-1 bg-white text-slate-600 text-xs font-bold rounded-lg border border-slate-200">{cat}</span>
               ))}
             </div>
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-display font-bold text-slate-900 tracking-tight">Tournament Central</h2>
        ...
        <div className="flex flex-col xs:flex-row gap-2 md:gap-4">
          {isAdmin && (
            <button 
              ...
              className="bg-primary text-white px-5 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-bold flex items-center gap-2 min-h-[40px] md:min-h-[44px] shadow-md shadow-indigo-500/20 hover:opacity-90 active:scale-[0.98] transition-all"
            >
              <Plus size={18} className="md:w-5 md:h-5" /> Add Event
            </button>
          )}
          <button className="bg-white px-4 md:px-6 py-2 md:py-3 flex items-center gap-2 text-primary font-bold min-h-[40px] md:min-h-[44px] shadow-sm border border-slate-100 rounded-xl md:rounded-2xl hover:shadow-md hover:bg-indigo-50 transition-all">
...
{loading ? (
             ...
             <div className="p-6 md:p-12 text-center bg-white rounded-[2rem] md:rounded-[40px] border-2 border-dashed border-slate-200 shadow-sm">
             ...
             events.map((event, i) => (
               <motion.div 
                 ...
                 className="bg-white rounded-3xl shadow-md overflow-hidden flex flex-col md:flex-row bento-card-hover border border-slate-100 relative group"
               >
                 {isAdmin && (
                   <button 
                     onClick={() => deleteEvent(event._id)}
                     className="absolute top-3 md:top-4 right-3 md:right-4 p-1.5 md:p-2 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity z-20 bg-white/80 rounded-xl shadow-sm"
                   >
                     <Trash2 size={14} className="md:w-4 md:h-4" />
                   </button>
                 )}
                 <div className={cn("w-full md:w-1", `bg-gradient-to-b ${event.color || 'from-primary to-indigo-600'}`)}></div>
                 <div className="flex-1 p-4 md:p-8">
                   <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4 md:mb-6 gap-2 md:gap-0">
                      <span className="px-3 py-1 bg-indigo-50 text-primary text-[10px] font-black rounded-lg uppercase tracking-widest border border-indigo-100 self-start">{event.title}</span>
                      <div className="flex items-center gap-1.5 md:gap-2 text-slate-400 font-bold text-[10px] md:text-[11px] uppercase"><Clock size={12} className="md:w-3.5 md:h-3.5 text-indigo-300" /> {event.time}</div>
                   </div>
                   ...
                   <div className="flex flex-col sm:flex-row items-center justify-center sm:gap-4 md:gap-6 lg:gap-8 py-2 md:py-4 px-1 md:px-2">
                      <div className="text-center flex-1 mb-3 sm:mb-0">
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-full mx-auto mb-2 md:mb-3 flex items-center justify-center text-xl md:text-2xl shadow-inner border border-slate-200">🐯</div>
                   ...
                   <div className="mt-4 md:mt-8 pt-4 md:pt-6 border-t border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
                      <div className="flex flex-col xs:flex-row xs:flex-wrap gap-2 md:gap-6 text-[10px] md:text-[11px] font-black uppercase tracking-[0.1em] text-slate-400">
                        <span className="flex items-center gap-1"><MapPin size={12} className="md:w-3.5 md:h-3.5 text-indigo-300" /> {event.venue}</span>
                        <span className="flex items-center gap-1"><Calendar size={12} className="md:w-3.5 md:h-3.5 text-indigo-300" /> {event.date}</span>
                      </div>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white rounded-3xl p-6 space-y-4 shadow-sm border border-slate-100">
              {[
                ...
                <div key={i} className="flex gap-4 p-4 rounded-3xl hover:bg-indigo-50/50 transition-colors cursor-pointer group border border-transparent hover:border-indigo-100">
            ...
            <div className="sporty-gradient rounded-3xl p-8 text-white shadow-xl shadow-indigo-500/20">
             <Trophy size={32} className="mb-4 opacity-70" />
             <h4 className="text-xl font-display font-bold mb-2">Join Member League</h4>
             <p className="text-white/90 text-sm mb-6 leading-relaxed">Exclusive for internal students. Weekly matches, ranking systems, and monthly prizes.</p>
             <button className="w-full bg-white text-primary font-bold py-3 rounded-xl text-sm hover:opacity-90 transition-opacity">LEARN MORE</button>
            </div>
        </div>
      </div>
