  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <button className="h-12 bg-white border border-secondary/20 px-6 rounded-2xl text-secondary hover:bg-secondary hover:text-white transition-all flex items-center gap-2 font-black text-[10px] uppercase tracking-widest shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="p-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400 hover:text-secondary"><ChevronLeft size={20} /></button>
          <div className="flex items-center gap-2">
            <span className="font-black text-lg text-slate-700">{months[month]} {year}</span>
          </div>
          <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))} className="p-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400 hover:text-secondary"><ChevronRight size={20} /></button>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-red-500" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Absent</span></div>
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-orange-400" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Leave</span></div>
        </div>
      </div>

      <div className="bento-card overflow-hidden border-secondary/10 shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[1240px]">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="sticky left-0 z-20 bg-slate-50/50 p-6 border-b border-slate-100 font-black text-[11px] uppercase tracking-widest text-slate-500 min-w-[260px]">Coach Name</th>
                {days.map(day => (
                  <th key={day} className="p-2 border-b border-slate-100 text-center font-black text-[10px] text-slate-400 min-w-[40px]">{day}</th>
                ))}
                <th className="p-4 border-b border-slate-100 font-black text-[11px] uppercase tracking-widest text-slate-500 text-center">Rate</th>
              </tr>
            </thead>
            <tbody>
              {coaches.map(coach => (
                <tr key={coach._id} className="hover:bg-slate-50/30 transition-colors group">
                  <td className="sticky left-0 z-20 bg-white group-hover:bg-slate-50 transition-colors p-6 border-b border-slate-50 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center text-secondary border border-orange-100 shadow-inner group-hover:bg-white transition-colors">
